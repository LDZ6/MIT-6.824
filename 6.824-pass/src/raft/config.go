...
func (cfg *config) ingestSnap(i int, snapshot []byte, index int) string {
	...
	DPrintf("server %v, snapshot last applied index: %v", i, lastIncludedIndex)
	return ""
}
	...
	for m := range applyCh {
		...
		if m.SnapshotValid {
			if rf.CondInstallSnapshot(m.SnapshotTerm, m.SnapshotIndex, m.Snapshot) {
				cfg.mu.Lock()
				DPrintf("server %v recive snapshot install last index: %v", i, m.SnapshotIndex)
				err_msg = cfg.ingestSnap(i, m.Snapshot, m.SnapshotIndex)
				cfg.mu.Unlock()
			} else {
				DPrintf("cond return false server %v recive snapshot install last index: %v", i, m.SnapshotIndex)
			}
			...
			DPrintf("server %v cfg last applied index: %v", i, cfg.lastApplied[i])
			cfg.mu.Unlock()
			...
			if (m.CommandIndex+1)%SnapShotInterval == 0 {
				...
				DPrintf("server %v call rf.Snapshot with index: %v", i, m.CommandIndex)
			}
		} // else {
		// Ignore other types of ApplyMsg.
		// }
...
func (cfg *config) start1(i int, applier func(int, chan ApplyMsg)) {
	...
	if cfg.saved[i] != nil {
		...
		if len(snapshot) > 0 {
			...
			DPrintf("server %v start1 with snapshot", i)
...
func (cfg *config) nCommitted(index int) (int, interface{}) {
	...
	for i := 0; i < len(cfg.rafts); i++ {
		...
		cfg.mu.Lock()
		// DPrintf("nCommitted, cfg.los[%v]: %v", i, cfg.logs[i])
...
func (cfg *config) one(cmd interface{}, expectedServers int, retry bool) int {
	...
	for time.Since(t0).Seconds() < 10 && !cfg.checkFinished() {
		...
		if index != -1 {
			...
			if !retry {
				cfg.t.Fatalf("one(index: %v, term: %v, cmd: %v) failed to reach agreement", index, term, cmd)
			}
		} else {
